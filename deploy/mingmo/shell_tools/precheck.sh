...
{
  echo
  log_info "check sysctl info" $check_result
  local i=1
  for (( i = 1; i <= ${#servers[*]}; i++ )); do
		server=${servers[$(($i-1))]}
    log_info "$server sysctl info: " $check_result
    ssh $precheck_user@$server " cat /etc/sysctl.conf" | tee -a $check_result
	done
  log_info ">>> Check sysctl info finished." $check_result
  return 0
}
...
{
  echo
  log_info "check limits info" $check_result
  local i=1
  for (( i = 1; i <= ${#servers[*]}; i++ )); do
		server=${servers[$(($i-1))]}
    log_info "$server limits info: " $check_result
    ssh $precheck_user@$server " cat /etc/security/limits.conf" | tee -a $check_result
	done
  log_info ">>> Check limits info finished." $check_result
  return 0
}
# 校验防火墙
check_firewalld()
{
  echo
  log_info "check firewalld info" $check_result
  local i=1
  for (( i = 1; i <= ${#servers[*]}; i++ )); do
		server=${servers[$(($i-1))]}
    log_info "$server limits info: " $check_result
    ssh $precheck_user@$server " sudo systemctl status firewalld" | tee -a $check_result
	done
  log_info ">>> Check firewalld info finished." $check_result
  return 0
}
